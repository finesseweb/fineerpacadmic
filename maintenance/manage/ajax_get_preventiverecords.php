...
$utility_id  = $_POST['sub_cat'];
$frequency_id  = $_POST['frequency_id'];

 //$sql="SELECT prevent.prevent_id,prevent.frequency_id FROM ".TB_PREF."prevent_maintain_entry AS prevent where prevent.sub_cat_id = '$utility_id'";
//$res = db_query($sql);
//$result = db_fetch_row($res);



//$sql23="SELECT * FROM ".TB_PREF."prevent_maintain_params where prevent_id = '$result[0]'";
//$res23 = db_query($sql23);
//$result23 = db_fetch_assoc($res23);
//echo $sql23;

///print_r($result23);

$sql2="SELECT utly.ut_param_id FROM ".TB_PREF."utility_parameters_master AS utly where utly.utly_sub_cat_id = '$utility_id' AND utly.frequency_id='$frequency_id' AND utly.type_maintenance_id=1";
...
<!-- <tr>
 ...
 <tr>
   ...
   $res=db_query($sql);
	...
	<select type="text" name="preventive[loc_code][]" id="items_id0" label="items0" onchange="stockqty(this.value,0);">
	<?php $sql= "SELECT location,store
   FROM ".TB_PREF."store_location"; 
   $res=db_query($sql);
     echo "<option>Select</option>";
	while($row = db_fetch($res))
	{ 
	?>  
	<option value="<?php echo $row['location']; ?>"><?php echo $row['store']; ?></option>
	<?php  
	} ?>
  </select>
	</td>
...
var cat = val;	
	 $.ajax({ 
			type: "POST",
			url:'<?php echo $path_to_root . "/maintenance/manage/ajax_get_sub_cat1.php";?>',
			data: { cat : cat}
		}).done( function( data ) { 
	     //	alert(data);
...
//alert(sub_cat);
	 $.ajax({ 
			type: "POST",
			url:'<?php echo $path_to_root . "/maintenance/manage/ajax_get_item_new.php";?>',
			data: { cat : cat ,sub_cat : sub_cat }
		}).done( function( data ) { 
	     //	alert(data);
