...
if ($Mode=='ADD_ITEM' && can_process()) 
{
	...
	$result2 = get_values($_POST['utly_type'],$_POST['utly_cat_id'],$_POST['utly_sub_cat_id'],$_POST['utilitys_id'],$_POST['type_maintenance_id'],$_POST['frequency_id']);
	$res = db_fetch_row($result2);
 
    if(($res[0] != $_POST['utly_type']) && ($res[4] != $_POST['type_maintenance_id']) && ($res[5] != $_POST['frequency_id'])){

	add_parameters($_POST['utly_type'],$_POST['utly_cat_id'],$_POST['utly_sub_cat_id'],$_POST['utilitys_id'],$_POST['type_maintenance_id'], $_POST['frequency_id'],$_POST['param_title'],$_POST['param_desc']);
	}
        
	else if(($res[0] == $_POST['utly_type']) && ($res[1] == $_POST['utly_cat_id']) && ($res[2] == $_POST['utly_sub_cat_id']) && ($res[4] == $_POST['type_maintenance_id']) && ($res[5] == $_POST['frequency_id'])){
	
	  add_paramitems($_POST['utly_cat_id'],$_POST['utly_sub_cat_id'],$_POST['utilitys_id'],$_POST['type_maintenance_id'], $_POST['frequency_id'],$_POST['param_title'],$_POST['param_desc']);
	}
	else {
...
///display_error($utly_type);
if($_POST['utly_type'] == 1){
...
if(list_updated('utly_type')){
	...
	$Ajax->activate('utly_cat_id');
	$Ajax->activate('utly_sub_cat_id');
	$Ajax->activate('utilitys_id');
	$Ajax->activate('frequency_id');
}
utly_categories_list_row(_("Category:"), 'utly_cat_id', null, false, true,$utly_type);
$category_id = $_POST['utly_cat_id'];
sub_category_list_row(_("Sub Category:"), 'utly_sub_cat_id', null, false, true, $category_id);
$sub_cat_name = $_POST['utly_sub_cat_id'];
//display_error($sub_cat_name);
if(list_updated('utly_cat_id')) {
	$Ajax->activate('utly_sub_cat_id');
	$Ajax->activate('utilitys_id');
	$Ajax->activate('frequency_id');
	$Ajax->activate('parameters_items');
}
if(list_updated('utly_sub_cat_id')) {
...
$utility_id = $sub_cat_name;
}
else {

//utility_list_row(_("Name :"), 'utilitys_id', null, false, true,$category_id,$sub_cat_name,$utly_type);
if(list_updated('utly_type')){
...
$utility_id = $_POST['utly_type'];


}
//display_error($utility_id);
if($_POST['utly_type'] == 1){
//utility_list_row(_("Name:"), 'utilitys_id', null, false, true,$category_id,$sub_cat_name,$utly_type);
maintenancetype_list_row(_("Type of Maintenance:"), 'type_maintenance_id', null, false, true,$utility_id);
} else {
    
maintenancetype_list_row_pro(_("Type of Maintenance:"), 'type_maintenance_id', null, false, true,$utility_id);
}
...
freq_multilist_row_pro(_("Frequency:"), 'frequency_id', null, false, true,$utility_id);
}
if(list_updated('utilitys_id')){
	$Ajax->activate('frequency_id');
	$Ajax->activate('parameters_table');
...
start_table(TABLESTYLE, "width=35%");
//display_error($_POST['frequency_id']);
$result2 = get_parameter_values($_POST['utly_type'],$_POST['utly_cat_id'],$_POST['utly_sub_cat_id'],$_POST['utilitys_id'],$_POST['type_maintenance_id'],$_POST['frequency_id']);
