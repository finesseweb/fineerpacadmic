...
function can_process($wo_details)
{
	global $SysPrefs;
             ...
             display_error(_("Please issue any item for the refrence."));
		set_focus('code');
		return false;
        }
	...
	if (($_POST['ProductionType'] == 1) && !$SysPrefs->allow_negative_stock())
	{
		...
		while ($row = db_fetch($result))
		{
			...
			{
                          ///display_error(input_num('quantity'));   
...
if ((isset($_POST['Process']) || isset($_POST['ProcessAndClose'])) && can_process($wo_details) == true)
{
	...
	if ($_POST['ProductionType'] == 0)
	 ...
	 $id = work_order_produce($_POST['selected_id'], $_POST['ref'], input_num('quantity'),
			$_POST['date_'], $_POST['memo_'], $close_wo,$_POST['item_code'],$_POST['item_name'],$_POST['item_des'],$_POST['category_id'],$_POST['sub_cat_name']);
...
$issue_ref_result = get_work_order_issue_ref($_POST['selected_id']);
$next_ref = db_fetch_row($issue_ref_result);
//-------------------------------------------------------------------------------------
...
ref_row(_("Reference:"), 'ref', '', !$next_ref[2]?'':$next_ref[2], false, ST_MANURECEIVE);
...
text_row("Item Code", "item_code", null, 50, 50, 'Item Code', null, null, false);
text_row("Item Name", "item_name", null, 50, 50, 'Item Code', null, null, false);
text_row("Item Description", "item_des", null, 50, 50, 'Item Code', null, null, false);
stock_categories_list_row(_("Category:"), 'category_id', null, false, true);
$category_id = $_POST['category_id'];
$Ajax->activate('_page_body');
//display_error($category_id);
sub_category_list_row(_("Sub Category:"), 'sub_cat_name', null, false, true, $category_id);
           
//text_row("Item Code", "item_code", null, 50, 50, 'Item Code', null, null, false);
//======for always quantity value as 1
//$dec = $dec>0?1:0;
small_qty_row(_("Quantity:"), 'quantity', null, null, null, $dec,true);
