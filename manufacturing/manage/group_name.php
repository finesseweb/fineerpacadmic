...
page(_($help_context = "Group Name"));
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	{
		$input_error = 1;
		display_error(_("The Group name cannot be empty."));
		set_focus('name');
	}
	...
	{
		$input_error = 1;
		display_error(_("The Group Description cannot be empty."));
	...
	if ($input_error != 1) 
	{
    	...
    	if ($selected_id != -1) 
    	{
    		update_group_name($selected_id, $_POST['name'], $_POST['description']);
			display_notification(_('Selected work center has been updated'));
    	} 
    	else 
    	{
    		add_group_name($_POST['name'], $_POST['description']);
...
$result = get_all_group_name(check_value('show_inactive'));
...
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
		...
		$myrow = get_group_name($selected_id);
...
text_row_ex(_("Group Name:"), 'name', 40,false,false,false,false,false,false,false,false,false,false,true);
text_row_ex(_("Description:"), 'description', 50,false,false,false,false,false,false,false,false,false,false,true);
